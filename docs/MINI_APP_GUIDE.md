...
ТЗ для Mini App:
Поток: Расклад → Тип карт → Колода → Перевернутые → Вопрос → Перемешать → Вытянуть → Толкование ВНУТРИ приложения.
Навигация: На каждом экране кнопки «⬅️ Назад» и «🏠 Основное меню».
Дизайн: Названия колод яркие (золотые), заголовок «Таро «12 Планет»».
Логика: Ответ ИИ отображается внутри приложения.
...
### 2. **Mini App — это удобный пульт ввода (20% проекта) и красочное отображение ИИ-интерпретации**
- Интерактивная клавиатура для отправки данных в бота и получение отвтета
- **Для Таро**: визуальная колода, свайп карт, анимация, задать вопрос для отправки выпавших карт в ИИ.
...
3. Пользователь выбирает расклад (Карта Дня, 1 карта, 3 карты, Кельтский крест...) - как в боте
...
8. **Бот получает массив карт**,  генерирует ИИ-интерпретацию и отображает всё в приложении
9. На каждом этапе присутствуют кнопки "назад" и "основное меню" для возврата или выхода в начало. 
...
| Интерпретация ИИ | ✅ В чате | ✅ Mini App | ✅ В mini app |
