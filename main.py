...
def parse_astrological_input(text: str) -> str:
    ...
    # ============================================================
    # Группы: 1=аспект, 2=планеты, 3=скобка1, 4=скобка2,
    #         5=град1, 6=знак1, 7=град2, 8=знак2
    aspect_pattern = (
        ...
        r"([><])\s*"                        # 4: скобка 2
        r"(\d+)([a-zA-Z]{3})\d+\s*-\s*"    # 5, 6: градусы и знак планеты 1
        r"(\d+)([a-zA-Z]{3})\d+"           # 7, 8: градусы и знак планеты 2
    )
    ...
    cusp_pattern = (
        r"\b(II|III|IV|V|VI|VII|VIII|IX|X|XI|XII|I)\s+"  # 1: римская цифра
...
async def cmd_start(message: types.Message):
    welcome_text = (
        ...
        "Мы полностью **отказались от встроенных расчетов** "
        "координат, городов и эфемерид внутри мессенджера. Опыт показывает, что с расчётами "
        "идеально справляются профессиональные программы — такие как **ZET** или **Sotis**.\n\n"
        "🎯 **Наша миссия — чистая, глубокая и бескомпромиссная интерпретация.**\n\n"
        "Я обучен строго на закрытой базе знаний нашей Школы. "
        "Я готов расшифровать для вас положения планет, знаков, домов, лунных дней.\n\n"
        "📝 **Как отправить запрос?**\n"
        "• **Обычным текстом**, например: _«Что означает Сатурн в Деве в 4 доме?»_ \n"
        "• **Прямым копипастом из программы ZET**, например:\n"
        "`Квадрат Сатурн-Нептун  > 91°19'<  20Vir16 - 21Sgr36`\n\n"
        "👉 Введите ваш вопрос или скопируйте строку аспектов из ZET ниже:\n\n"
        "⚠️ **Рекомендуются: НЕ БОЛЕЕ одного-двух аспектов за один запрос**\n\n"
        "ℹ️ Бот работает на бесплатном сервере: если он не отвечает больше минуты — "
        "сервер «уснул» после простоя. Нажмите кнопку ниже, подождите ~10–15 минут, пока откроется "
        "страница, и повторите команду /start. \n\n"
        "или откройте ссылку: https://astro-bot-b8m8.onrender.com/ и подождите ~3–4 минуты. "
    )
...
async def handle_user_input(message: types.Message):
    ...
    if any(word in raw_text for word in forbidden_objects):
        refusal_text = (
            "⚠️ **Уведомление Школы Астрологии «12 Планет»**\n\n"
            "Вы упомянули что-то из: астероиды, фиктивные точки (Лилит, Лунные узлы), жребии (парсы), туманности или другие объекты, Раху / Кету, экзальтации или падения, "
