...
// Добавьте это В НАЧАЛО файла mini-app.js, после const tg = window.Telegram.WebApp;
...
const SPREAD_CONFIGS = {
    ...
    choice: { name: 'Выбор пути', count: 7, positions: [
...
async function drawCards() {
    console.log('🎴 drawCards() вызвана');
    console.log('AppState:', AppState);
    
    const config = SPREAD_CONFIGS[AppState.currentSpread];
    if (!config) {
        console.error('❌ Не найдена конфигурация для расклада:', AppState.currentSpread);
        tg.showAlert('Ошибка: тип расклада не выбран. Вернитесь и выберите расклад.');
        return;
    }
    ...
    try {
        ...
        console.log('🃏 Получены карты:', result);
